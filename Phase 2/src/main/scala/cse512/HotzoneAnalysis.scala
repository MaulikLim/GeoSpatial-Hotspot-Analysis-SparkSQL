...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    //Run a group by aggregation and sort by "rectangle"
    var resultDf = spark.sql("select * from joinResult")
    resultDf = resultDf.groupBy(resultDf("rectangle")).count().orderBy(resultDf("rectangle"))
