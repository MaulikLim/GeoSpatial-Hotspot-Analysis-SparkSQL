...
object HotcellAnalysis {
  ...
  def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame = {
    ...
    val temp = clusters.agg(sum(clusters("count")).as("sum"), sum(clusters("count") * clusters("count").as("squaredsum"))).head()
    val trips = temp.getLong(0)
    val squaredSum = temp.getLong(1)
    val mean = trips / numCells
    val S = Math.sqrt((squaredSum / numCells) - (mean * mean))
    System.out.println("Mean: " + mean + " S: " + S + " num: " + numCells)
    ...
    val num = countX.as("c").join(neighborCount.as("n")).where("c.x = n.x and c.y = n.y and c.z = n.z")
      .select(countX("x").as("x"),
        countX("y").as("y"),
        countX("z").as("z"),
        (countX("count") - lit(mean) * neighborCount("w")).as("num"))
    //    num.show()
